(function ($) {
    ...
    $t.fx.zoom.prototype = {
        play: function (options, end) {
            ...
            var endValues = {
                'max-width': resizeElement.width(),
                'min-height': resizeElement.height() + 5,
                left: parseInt($element.css('left')),
                top: parseInt($element.css('top'))
            };
            $element.css({
                left: endValues.left,
                top: endValues.top - 50
            }).animate({
                left: endValues.left,
                top: endValues.top
    ...
    $t.window.prototype = {
        ...
        overlay: function (visible) {
            var overlay = $('body').find('.t-overlay');
            //if (overlay.length === 0)
