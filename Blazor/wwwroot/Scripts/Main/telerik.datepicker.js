(function ($) {
    ...
    $t.dateView.prototype = {
            ...
            if (!sharedCalendar) {
                sharedCalendar = $($t.calendar.html(new $t.datetime(this.focusedValue), this.selectedValue ? new $t.datetime(this.selectedValue) : null, new $t.datetime(this.minValue), new $t.datetime(this.maxValue)))
                                ...
                                .tCalendar({
                                    ...
                                    minPDate: new $t.PersianDate_(this.minPDate),
                                    maxPDate: new $t.PersianDate_(this.maxPDate),
                                    PViewdMonth: null
                                });
        ...
        _reassignSharedCalendar: function () {
            ...
            if (sharedCalendar.data('associatedDateView') != this) {
                ...
                calendar.minPDate = new $t.PersianDate_(this.minPDate);
                calendar.maxPDate = new $t.PersianDate_(this.maxPDate);
                //calendar.goToView(0, this.focusedValue, new $t.PersianDate_($.fn.tDatePicker.defaults.pDate));

                sharedCalendar
                    .unbind('change')
                    .bind('change', $.proxy(function (e) {
                        this.onChange(null, e.pDate);
                    }, this))
                    .unbind('navigate')
                    .bind('navigate', $.proxy(function (e) {
                        ...
                        $t.calendar.focusDate(focusedValue, viewIndex, sharedCalendar, e.direction, "1390/01/08");
                ...
                $t.calendar.focusDate(this.focusedValue, calendar.currentView.index, sharedCalendar, "1390/01/07");
            }
        },
        ...
        open: function (position) {
            ...
            var calendar = this._getCalendar();
            var viewIndex = calendar.currentView.index;
            if (!sharedCalendar.is(':visible')) {
                var tempPDate = this.pDate;
                if (tempPDate == null)
                    tempPDate = this.currentPDate;
                calendar.goToView(viewIndex, this.focusedValue, new $t.PersianDate_(tempPDate)).value(this.selectedValue);
            }
            $t.calendar.focusDate(this.focusedValue, calendar.currentView.index, sharedCalendar, "1390/01/04");
            $t.fx._wrap($calendar).css('zIndex', position.zIndex).show();
            $t.fx.play(this.effects, $calendar, { direction: 'bottom' });
        },

        close: function () {
            var date = this._getCalendar().pDate;
            var str = date.Year + '/';
            if (date.Month < 10)
                str += '0';
        ...
        navigate: function (e) {
            ...
            if (isNavProcessed) {
                ...
                $t.calendar.focusDate(date.toDate(), viewIndex, $calendar, isFuture, "1390/01/12");
    ...
    $t.datepicker = function (element, options) {
        ...
        $(element).mouseover(function () {
            $(element).parent().addClass('t-hover');
        });
        $(element).mouseleave(function () {
            $(element).parent().removeClass('t-hover');
        });
        ...
        var $element = this.$element = $(element)
                    ...
                    .bind({
                        ...
                        blur: function (e) {
                            $(e.target).parent().removeClass('t-state-selected');
                        },
                        focus: $.proxy(function (e) {
                            self.oldValue = self.value();
                            $(e.target).parent().addClass('t-state-selected');
                            if (this.openOnFocus) {
                                this._open();
                            }
        ...
        if (this.selectedValue) {
            this.pDate = new $t.PersianDate_(this.selectedValue).toStringDate();
            this._updateP(this.pDate);
        }
        else {
            var date = new Date();
            date = Date.UTC(date.getFullYear(), date.getMonth(), date.getDate(), 0, 0, 0, 0);
            this.pDate = $t.PersianDate_(date).toStringDate();
        ...
        this.dateView = new $t.dateView({
            ...
            minPDate: this.minPDate,
            maxPDate: this.maxPDate,
            pDate: this.pDate,
            currentPDate: this.currentPDate,
            effects: this.effects,
            isRtl: $element.closest('.t-rtl').length,
            onChange: $.proxy(function (value, pDate) {

                if (pDate !== null) {

                    this._updateP(pDate.toStringDate());
                    this._close();
                }
                else
                    alert("تاریخ درست انتخاب نشده است.");
            }, this)
        });

        this.inputValue = $element.val();
        //var value = this.selectedValue || this.inputValue;
        var value = this.pDate || this.inputValue;
        this.pDate = value;
    ...
    $t.datepicker.prototype = {
        ...
        showErrorMessage: function (flag) {
            ...
            if (this.value() == '') {
                if ($(this.element).val() != '') {
                    if (flag != false)
                        $t.showErrorMessage(id, 'فرمت تاریخ صحیح نیست');
                    return true;
                }
            }
            else{
                if (this.inputValue < this.minPDate || this.inputValue > this.maxPDate) {
                    if (flag != false)
                        $t.showErrorMessage(id, this.errMsg);
                    this.focus();
                    return true;
                }
                if (this.compareType) {
                    var other = $('#' + this.otherPropery + '_Date').getDatePicker();
                    if ($t.compare(this.compareType, this.value(), other.value()) != true) {
                        if (flag != false)
                            $t.showErrorMessage(id, this.errMsg);
                        return true;
                    }
        ...
        _updateP: function (val) {
            var text = val;
            val = new $t.PersianDate_().Parse(val);
            var isNull = false;
            if (val == null || val.Year == null) {
                isNull = true;
                this.inputValue = null;
            }
            else {
                text = val.toStringDate();
                this.pDate = text;
                $.fn.tDatePicker.defaults.pDate = text;
        ...
        _keydown: function (e) {
            ...
            setTimeout($.proxy(function () {
                ...
                this._updateP(this.$element.val());
                var star = $(this.element).parent().find('.t-star');
                ...
                if (tempValue == '')
                    tempValue = null;
                if (this.oldValue != tempValue) {
                    $t.trigger(this.element, "changeValue", null);
                    this.oldValue = tempValue;
                    if (this.isSes == true) {
                        var grid = null;
                        if (this.gridName) {
                            grid = $('#' + this.gridName).data('tGrid');
                            if (!grid)
                                throw new Error('خطا:grid with name ' + this.gridName + ' not exist in form');
                        }
                        else
                            grid = $t.getGrid(null, this.element);
                        if (grid) {
            ...
            if (keyCode == 9 && this.onChange && this.inputValue != this.$element.val())
                this.onChange();
            if (keyCode == 27) {
                this._close();
            } else if (e.altKey) {
        ...
        valueSetP: function (value) {
            var date = new Date();
            if (value != null && value != "") {
                value = value.substr(6);
                value = value.substr(0, value.length - 2);
                date.setTime(value);
                //value = $t.PersianDate_(date).toStringDate();
            }
            else
                value = null
            this.dateView.pDate = value;
            this.pDate = value;
        ...
        open: function () {
            ...
            if (value == null)
                this.dateView.pDate = $t.PersianDate_(new Date()).toStringDate();
            else
                this.dateView.pDate = this.pDate;
            this.dateView.open({
        ...
        close: function () {
            this.dateView.close();
            this.value(this.dateView.pDate);
        },
    ...
    $.fn.tDatePicker.defaults = {
        ...
        shortYearCutOff: 30,
        showButton: true,
        buttonTitle: 'برای نمایش تقویم کلیک نمائید',
        enabled: true,
        openOnFocus: false
    };
